				...
				<Box>
					...
					<div className={styles.btnGroup}>
						<div className="btn-group">
							...
							<Link className="btn btn-default" to={`/${path}/projects`}>Земля</Link>
							<Link className="btn btn-default" to={`/${path}/projects`}>Нерухомість</Link>
							<Link className="btn btn-default" to={`/${path}/projects`}>Кооперація</Link>
						</div>
					</div>
					<div style={{
						'height': "400px"
											...
											<Popup>
												<span onClick={(event) => {
													history.push(`${this.props.hostEnable
														? ''
														: '/' + this.props.community.get('slug')}/invest-map/projects/detail/${item.get('id')}`)
												}}>{item.get('name')}</span>
											</Popup>
										</Marker>
									)
					...
					<Pagination counts={this.props.investments.get('count')} path={`${this.props.hostEnable
						? ''
						: '/' + this.props.community.get('slug')}/invest-map/projects/`} matched={this.props.match.params.page} limits={10}/>
